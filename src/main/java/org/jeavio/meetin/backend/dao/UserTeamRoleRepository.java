...
public interface UserTeamRoleRepository extends CrudRepository<UserTeamRole, Integer> {
	...
	@Query("SELECT new org.jeavio.meetin.backend.dto.UserInfo(utr.user.id,utr.user.empId,utr.user.firstName,utr.user.lastName,utr.user.email) FROM UserTeamRole utr WHERE utr.role.role = 'super_admin' ")
